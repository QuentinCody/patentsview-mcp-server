 ...
 */
function mergeSchemaHints(serverHints, clientHints) {
...
function deduplicateCompositeIndexes(indexes) {
    const seen = new Set();
    const result = [];
...
export class RestStagingDO extends DurableObject {
     ...
     */
    storeProvenance(context) {
        this.ctx.storage.sql.exec(`CREATE TABLE IF NOT EXISTS _staging_metadata (
        ...
        if (context) {
            this.ctx.storage.sql.exec(`INSERT INTO _staging_metadata (tool_name, server_name, args_json, api_url) VALUES (?, ?, ?, ?)`, context.toolName ?? null, context.serverName ?? null, context.args ? JSON.stringify(context.args) : null, context.apiUrl ?? null);
        }
    }
    /**
     * Update provenance with row counts after materialization.
     */
    updateProvenanceRowCounts(inputRows, storedRows, failedRows, warnings) {
        try {
            this.ctx.storage.sql.exec(`UPDATE _staging_metadata SET input_rows = ?, stored_rows = ?, failed_rows = ?, warnings_json = ? WHERE id = (SELECT MAX(id) FROM _staging_metadata)`, inputRows, storedRows, failedRows, warnings.length > 0 ? JSON.stringify(warnings) : null);
        }
        catch {
     ...
     */
    persistInferredSchema(schema) {
        try {
            this.ctx.storage.sql.exec(`CREATE TABLE IF NOT EXISTS _inferred_schema (
					id INTEGER PRIMARY KEY,
					schema_json TEXT
				)`);
            this.ctx.storage.sql.exec(`INSERT OR REPLACE INTO _inferred_schema (id, schema_json) VALUES (1, ?)`, JSON.stringify(schema));
        }
        catch {
    ...
    async handleProcess(request) {
        ...
        const container = (raw !== null && typeof raw === "object" ? raw : {});
        ...
        this.ctx.storage.sql.exec(`CREATE TABLE IF NOT EXISTS payloads (
				id INTEGER PRIMARY KEY AUTOINCREMENT,
				root_json TEXT,
				created_at TEXT DEFAULT CURRENT_TIMESTAMP
			)`);
        const jsonStr = await this.chunking.smartJsonStringify(data, this.ctx.storage.sql);
        this.ctx.storage.sql.exec(`INSERT INTO payloads (root_json) VALUES (?)`, jsonStr);
        const count = this.ctx.storage.sql.exec(`SELECT COUNT(*) as c FROM payloads`).one()
            ?.c ?? 0;
        return this.jsonResponse({
            success: true,
            table_count: 1,
    ...
    async handleQuery(request) {
        const raw = await request.json();
        const body = (raw !== null && typeof raw === "object" ? raw : { sql: "" });
    ...
    async handleQueryEnhanced(request) {
        const rawEnhanced = await request.json();
        const body = (rawEnhanced !== null && typeof rawEnhanced === "object" ? rawEnhanced : { sql: "" });
        const res = this.ctx.storage.sql.exec(body.sql);
        ...
        for (const row of rows) {
            ...
            for (const [k, v] of Object.entries(row)) {
                if (typeof v === "string" && this.chunking.isContentReference(v)) {
                    ...
                    const content = await this.chunking.retrieveChunkedContent(id, this.ctx.storage.sql);
                    try {
    ...
    async handleSchema() {
        ...
        try {
            ...
            if (schemaResults.length > 0) {
                ...
                if (schemaRow?.schema_json) {
                    inferredSchema = JSON.parse(schemaRow.schema_json);
                }
            }
        }
        catch {
        ...
        const tableResults = this.ctx.storage.sql
            .exec(`SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' AND name NOT LIKE '_staging_%' AND name != '_inferred_schema'`)
            .toArray();
        ...
        const relationships = inferredSchema
    ...
    async handleRegister(request) {
        const rawRegister = await request.json();
        const body = (rawRegister !== null && typeof rawRegister === "object" ? rawRegister : {});
...
//# sourceMappingURL=rest-staging-do.js.map